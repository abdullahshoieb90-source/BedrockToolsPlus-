...
- **Tracers** draws a snapline to each entity as world-space geometry in the same render pass as the boxes, so the line physically ends inside the hitbox and cannot slide off it while you turn the camera. The origin is your own feet (**Bottom**) or the camera, i.e. the screen center (**Crosshair**); it is styled by **Tracer Color** and stays hairline. An entity that is off the screen keeps its snapline — the line points the way.
...
- **Nametag** and **Health** are the remaining HUD furniture (the launcher owns the font, which is also why Arabic names render correctly): **Nametag Color** and **Nametag Scale** style the name, **Health** adds the value plus a green → yellow → red bar, both anchored above the box through the module's projection. **Fov** matches that projection to your in-game field of view (default 60, a *vertical* FOV widened by the surface aspect); nudge it if those two labels sit too close to the middle of the screen or overshoot their target. The boxes, tracers and distance readouts do not depend on it.
...
The world-space builders (box edges, corner brackets, faces, tracers, the billboarded distance glyphs), the camera basis and the projection are plain math in `esp_geometry.hpp`, and the tessellator/material plumbing lives in `overlay_mesh.hpp`; both are pure enough to run on the host, so `tests/esp_geometry_test.cpp` covers the math and `tests/esp_render_test.cpp` drives the real render hook with stand-in actors and checks the emitted vertices and draw commands — including that turning the view leaves the box geometry bit-identical and cannot slide the readout off the hitbox.
