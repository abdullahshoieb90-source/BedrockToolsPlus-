...
- **HUD Fallback Always** (off by default) draws the boxes on the HUD layer even while the world pass reports it is drawing. It exists for one case: HUD Diagnostics says "world pass live" and the screen is still empty, which means the pass runs but its output is not visible. Turn this on and the overlay comes off that pass completely.
...
- The fallback finds the player the way the HUD modules do — `ClientInstance::getLocalPlayer()` through a vtable slot — whenever the actor tick has not delivered a player pointer, so it does not depend on that one signature resolving either.
