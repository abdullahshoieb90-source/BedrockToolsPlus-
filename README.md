...
- **Tracers** draws a snapline that ends inside the entity's hitbox, so it cannot slide off it while you turn the camera. The origin is your own feet (**Bottom**) or the screen center (**Crosshair**), and the two need different paths for a geometric reason: **Bottom** is world-space geometry handed to the game in the same render pass as the boxes, while a line that starts at the camera lies on a single view ray — the game's projection collapses all of it onto one pixel, which is why the crosshair snapline used to disappear when it was selected. **Crosshair** is therefore HUD furniture, drawn from the exact middle of the screen to the projected hitbox. Both are styled by **Tracer Color** and stay thin, and an entity that is already off the screen keeps a snapline pointing the way.
...
- **Nametag** and **Health** are the remaining HUD furniture (the launcher owns the text): **Nametag Color** and **Nametag Scale** style the name, **Health** adds the value plus a green → yellow → red bar, both anchored above the box through the module's projection. Each label picks its own face and is measured on it: the game's pixel font (`resources/minecraft.ttf`, registered once per package and shared with Effect Display) whenever it carries the text, because the font's cell widths are what the centering is computed from, and the launcher's default font for a script the pixel font has no glyph for — Arabic, Hebrew, CJK, emoji — which is also the only one that shapes right-to-left names instead of drawing a row of replacement boxes. The name is cleaned before it is drawn — the section-sign markup codes a server or a nick add-on pads it with, and the invisible zero-width and bidi characters a right-to-left name arrives wrapped in, are exactly what a plain HUD font would paint as extra characters beside the label; a doubled sign, which is the game's own escape for a literal **§**, survives as that one character — and it is centered on a width measured in *glyphs* rather than bytes, so a multi-byte name sits above the head instead of to the left of it. **Fov** matches that projection to your in-game field of view (default 70, Bedrock's own `gfx_fov`, a *vertical* FOV widened by the surface aspect); nudge it if those two labels sit too close to the middle of the screen or overshoot their target. The boxes, the feet-origin tracers and the distance readouts do not depend on it.
...
The world-space builders (box edges, corner brackets, faces, tracers, the billboarded distance glyphs), the camera basis, the projection, the nametag text cleanup, which font it ends up drawn with and the glyph-width measurement that centers it are plain math in `esp_geometry.hpp`, and the tessellator/material plumbing lives in `overlay_mesh.hpp`; both are pure enough to run on the host, so `tests/esp_geometry_test.cpp` covers the math — including that a camera-origin line really is degenerate and where the crosshair tracer goes instead — and `tests/esp_render_test.cpp` drives the real render hook with stand-in actors and checks the emitted vertices and draw commands, including that turning the view leaves the box geometry bit-identical, cannot slide the readout off the hitbox, and never blanks the labels of the frame around one actor with a broken collision box.
