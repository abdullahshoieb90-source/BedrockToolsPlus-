...
- **Nametag**, **Health** and **Distance** are HUD furniture (the launcher owns the font, which is also why Arabic names render correctly), so they are projected from world space and anchored above the box: **Nametag Color** and **Nametag Scale** style the name, **Health** adds the value plus a green → yellow → red bar, and **Distance** prints the range from the camera in meters. **Tracers** draws a snapline to each entity, anchored at the bottom of the screen (**Bottom**) or at the crosshair (**Crosshair**) and styled by **Tracer Color**. **Fov** matches that projection to your in-game field of view (default 60, a *vertical* FOV widened by the surface aspect); nudge it if the *labels* sit too close to the middle of the screen or overshoot their target. The boxes do not depend on it.
...
The world-space builders (box edges, corner brackets, faces), the camera basis and the projection are plain math in `esp_geometry.hpp`, and the tessellator/material plumbing lives in `overlay_mesh.hpp`; both are pure enough to run on the host, so `tests/esp_geometry_test.cpp` covers the math and `tests/esp_render_test.cpp` drives the real render hook with stand-in actors and checks the emitted vertices and draw commands — including that turning the view leaves the geometry bit-identical.
