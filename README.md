...
- The movement input scheme is forced to the player-relative one for as long as the camera is away from the body. Modern Bedrock interprets W/S/A/D relative to the **camera** (`MoveInputComponent` flags `IsCameraRelativeMovementEnabled` / `IsRotControlledByMoveDirection`), so freezing the rotation alone would leave the player walking wherever the camera looks. Forcing the two flags off keeps the displacement on the locked direction (W/S along the facing, A/D strafe, body never turned by the movement), and the scheme the player had before is restored on release.
