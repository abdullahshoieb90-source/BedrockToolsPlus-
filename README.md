...
- **Tracer** (off by default) draws one line per highlighted container from you to the middle of its box, in that container's group color and through walls when **Through Walls** is on. **Tracer Origin** picks the anchor — **Camera** converges every line on your crosshair, **Feet** starts them at the block you stand in, which reads better in third person — and **Tracer Thickness**/**Tracer Opacity** style them. A tracer keeps the same width on screen at any range (its world-space strip widens with distance instead of thinning to a sub-pixel hair), its start is clipped just in front of the eye plane so the line always rasterises, and containers behind you are skipped. Tracers share **Max Boxes** and the group toggles, so they never draw to a hidden or out-of-range container.
