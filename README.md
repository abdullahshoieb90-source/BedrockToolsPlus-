...
- **Nametag** and **Health** are the remaining HUD furniture (the launcher owns the font, which is also why Arabic names render correctly): **Nametag Color** and **Nametag Scale** style the name, **Health** adds the value plus a green → yellow → red bar, both anchored above the box through the module's projection. The name is cleaned before it is drawn — the section-sign markup codes a server or a nick add-on pads it with, and the invisible zero-width and bidi characters a right-to-left name arrives wrapped in, are exactly what a plain HUD font would paint as extra characters beside the label — and it is centered on a width measured in *glyphs* rather than bytes, so a multi-byte name sits above the head instead of to the left of it. **Fov** matches that projection to your in-game field of view (default 70, Bedrock's own `gfx_fov`, a *vertical* FOV widened by the surface aspect); nudge it if those two labels sit too close to the middle of the screen or overshoot their target. The boxes, the feet-origin tracers and the distance readouts do not depend on it.
