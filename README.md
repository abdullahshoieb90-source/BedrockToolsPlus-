...
**Storage ESP** highlights storage blocks around you: chests, copper chests (every oxidation and waxed stage), trapped chests, ender chests, shulker boxes (all colors), barrels, hoppers, furnaces (including blast furnaces and smokers), dispensers and droppers. It is entirely client-side and only looks at blocks the world already has loaded.
...
- **Scan Radius** (8–256 blocks, about 16 chunks at max) and **Scan Height** (2–128 blocks above and below your layer) set how far the module looks — boxes stay visible from very far away instead of popping out at close range. **Scan Speed** (Relaxed, Balanced, Fast, Instant) decides how many block positions each game tick is allowed to check; the scan is split across ticks, so it never stalls a frame, and the closest chunks are always swept first. At long ranges prefer **Fast** or **Instant** so the far edge of the area fills in quickly. The sweep keeps its progress while you walk instead of restarting every few steps, always continuing with the nearest chunk it has not scanned yet, so every chunk is visited exactly once per pass and the far edge is reached even while moving — only a teleport restarts it from the nearest chunks.
...
- **Outline**, **Line Thickness**, **Fill** and **Fill Opacity** style the boxes the same way Block Outline does, with Android-safe camera-facing line geometry. **Model Sized Boxes** follows the smaller chest (wooden, copper, trapped, ender) and hopper models instead of painting the whole voxel.
...
- **Tracer** (off by default) draws one line per highlighted container from you to the middle of its box, in that container's group color and through walls when **Through Walls** is on. **Tracer Origin** picks the anchor — **Bottom** starts the line just under the eye, which keeps it off your line of sight so it crosses the lower screen on its way to the container instead of collapsing into a pixel at the crosshair, and **Feet** starts it at the block you stand in, which reads better in third person — and **Tracer Thickness**/**Tracer Opacity** style them. A tracer keeps the same width on screen at any range (its world-space strip widens with distance instead of thinning to a sub-pixel hair), its start is clipped just in front of the eye plane so the line always rasterises, containers behind you are skipped, and a line that would land on a single view ray is dropped instead of drawn as a dot. Tracers share **Max Boxes** and the group toggles, so they never draw to a hidden or out-of-range container.
- **Rainbow**/**Rainbow Speed** and **Pulse**/**Pulse Speed** animate the group colors (tracers included), and **Max Boxes** caps how many boxes are drawn per frame, nearest first, so huge bases stay cheap.

Blocks that are not loaded yet can never be highlighted, so the effective range is still bounded by what the world has loaded around you — but within loaded chunks the radius now reaches "whole base" distances instead of fading out nearby.
