...
// Diagnostic-only logging -- see the matching comment in hitbox.cpp. This
// does not change any rendering behavior; it only turns silent early-returns
// into something visible in logcat.
#if defined(__ANDROID__)
#include <android/log.h>
#define BTP_BO_LOGD(...) __android_log_print(ANDROID_LOG_DEBUG, "BTP-BlockOutline", __VA_ARGS__)
#define BTP_BO_LOGW(...) __android_log_print(ANDROID_LOG_WARN,  "BTP-BlockOutline", __VA_ARGS__)
#else
#define BTP_BO_LOGD(...) ((void)0)
#define BTP_BO_LOGW(...) ((void)0)
#endif
...
void renderBlockOutline(void* levelRenderer, void* screenContext) {
    ...
    static bool loggedReason = false;
    if (!loggedReason) {
        if (!levelRenderer || reinterpret_cast<std::uintptr_t>(levelRenderer) < 0x1000 ||
            ...
            BTP_BO_LOGW("render hook active but levelRenderer/screenContext looks invalid (renderer=%p screen=%p)", levelRenderer, screenContext);
        } else if (!s_tessBegin || !s_tessColor || !s_tessVertex || !s_renderMesh) {
            BTP_BO_LOGW("render hook active but a Tessellator/RenderMesh pointer is null -- nothing will ever draw");
        } else if (!s_levelGetHitResult) {
            BTP_BO_LOGW("LevelGetHitResult signature never resolved -- the target block can never be found, so nothing ever draws");
        } else {
            BTP_BO_LOGD("render hook active and all required pointers are resolved -- rendering should be visible while looking at a block");
        }
        loggedReason = true;
    }
...
void BlockOutlineModule::onInit() {
    ...
    if (materialGroup) {
        ...
        if (groupAddress) {
            ...
            BTP_BO_LOGW("RenderMaterialGroupCommon resolved (%p) but ADRP decode failed; materials will use the fallback overlay only", (void*)materialGroup);
        }
    }
    ...
    BTP_BO_LOGD(
        "onInit: RenderLevel=%p TessBegin=%p TessColor=%p TessVertex=%p RenderMesh=%p "
        "LevelGetHitResult=%p MaterialGroup=%p",
        m_patchTarget, (void*)s_tessBegin, (void*)s_tessColor, (void*)s_tessVertex,
        (void*)s_renderMesh, (void*)s_levelGetHitResult, (void*)s_renderMaterialGroup);

    if (!m_patchTarget) BTP_BO_LOGW("RenderLevel signature not found -- Block Outline cannot install its render hook at all");
    if (!s_tessBegin || !s_tessColor || !s_tessVertex || !s_renderMesh)
        BTP_BO_LOGW("A Tessellator/RenderMesh signature is missing -- the render hook will run but draw nothing every frame");
    if (!s_levelGetHitResult) BTP_BO_LOGW("LevelGetHitResult not found -- the targeted block can never be detected");
}

void BlockOutlineModule::applyPatch() {
    if (m_patched) { BTP_BO_LOGD("applyPatch: already patched, skipping"); return; }
    if (!m_patchTarget) { BTP_BO_LOGW("applyPatch: no RenderLevel target resolved, cannot install hook"); return; }
    ...
    if (!m_patched) {
        BTP_BO_LOGW("applyPatch: hooks::install FAILED on target=%p (already hooked in a conflicting way, or rejected by the hook backend)", m_patchTarget);
    } else {
        BTP_BO_LOGD("applyPatch: hook installed on target=%p orig=%p", m_patchTarget, (void*)s_renderLevelOriginal);
    }
}
