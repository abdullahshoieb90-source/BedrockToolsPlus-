...
// Proven actor-enumeration path: the same ActorManager::getRuntimeActorList
// call that Tablist and DebugMenu use (returns every runtime actor).
...
static bedrocktools::sdk::Vec3 getActorPos(void* actor) {
    uintptr_t actorAddr = (uintptr_t)actor;
    uintptr_t svc = *(uintptr_t*)(actorAddr + bedrocktools::sdk::offsets::Actor::mStateVectorComponent);
    if (!svc || svc < 0x1000) return {0.f, 0.f, 0.f};
    return *(bedrocktools::sdk::Vec3*)svc;
}
...
static void _renderLevel_hook(void* _this, void* screenContext, void* a3) {
    ...
    if (s_getRuntimeActorList) {
        ...
        void* actorManager = (levelPtr && levelPtr > 0x1000)
        ...
        if (actorManager) {
            ...
            for (void* ent : actors) {
                ...
                bedrocktools::sdk::Vec3 epos = getActorPos(ent);
                float ddx = epos.x - localPos.x;
                float ddy = epos.y - localPos.y;
                float ddz = epos.z - localPos.z;
                if ((ddx * ddx + ddy * ddy + ddz * ddz) > 900.0f) continue;

                bool isPlayer = s_actorIsPlayer
                    ? s_actorIsPlayer(ent)
                    : hasCategory(ent, bedrocktools::sdk::offsets::ActorCategories::IsPlayer);
                if (isPlayer && !g_hitboxMod->showPlayers) continue;
                if (!isPlayer && (!g_hitboxMod->showEntities ||
                    !hasCategory(ent, bedrocktools::sdk::offsets::ActorCategories::IsMob))) continue;
