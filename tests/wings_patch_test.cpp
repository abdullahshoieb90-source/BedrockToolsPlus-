...
int main() {
    ...
    // ------------------------------------------------------------------
    // Perspective-id driven first-person guard
    // ------------------------------------------------------------------
    // The render pass now reads the perspective id captured by the
    // GetPerspective hook. perspective == 0 means First Person, and that
    // is the case the user reported as broken (wings on top of the face).
    // We drive the cache directly from the test because the host cannot
    // install a hook on Bedrock's vtable.
    std::printf("perspective id driven first-person guard\n");
    {
        // First-person: hide the wings, regardless of where the camera is.
        WingsModule::setPerspectiveIdForTest(0);
        check(WingsModule::isFirstPersonPerspective(),
              "perspective id 0 (First Person) hides the wings");
        check(WingsModule::currentPerspectiveId() == 0,
              "currentPerspectiveId() reports the cached value");

        // Even with the camera pulled far back outside the AABB (which
        // would pass the old AABB test alone), the perspective guard must
        // still hide the wings in first-person. This is the exact case
        // that used to leak the wings in front of the camera in some
        // front-facing transitions: the AABB test passed, the wings
        // drew, and the camera then orbited around to face the player.
        check(WingsModule::isFirstPersonPerspective(),
              "perspective 0 hides the wings even with a far-back camera");
    }
    {
        // Third-person back: show the wings.
        WingsModule::setPerspectiveIdForTest(1);
        check(!WingsModule::isFirstPersonPerspective(),
              "perspective id 1 (Third Person Back) shows the wings");
    }
    {
        // Third-person front: still show the wings. The AABB test is what
        // hides them when the camera is in front of the player's body in
        // this mode (the camera ends up inside the box from the player's
        // side); the perspective hook just confirms "not first-person".
        WingsModule::setPerspectiveIdForTest(2);
        check(!WingsModule::isFirstPersonPerspective(),
              "perspective id 2 (Third Person Front) is not first-person");
    }
    {
        // Unknown / future perspective ids: do not falsely report
        // first-person (would hide the wings) and do not crash.
        WingsModule::setPerspectiveIdForTest(7);
        check(!WingsModule::isFirstPersonPerspective(),
              "unknown perspective ids are not treated as first-person");
    }

    // Restore third-person for the rest of the tests so the AABB-based
    // checks below still describe third-person.
    WingsModule::setPerspectiveIdForTest(1);

    // ------------------------------------------------------------------
    // Torso / chest Y-anchor for the wing rig
    // ------------------------------------------------------------------
    // The wing shoulder pivots live at y = 21 px in model space (= 21/16
    // ~ 1.31 blocks above the rig origin). Anchoring at the feet (y = 0)
    // would drag the wings 1.31 blocks below the back, putting the root
    // pivots inside the player's legs. The fix binds the rig to the
    // torso bone, computed as feetY + kTorsoAnchorRatio * height, which
    // hits the upper chest for a standing player and scales while
    // sneaking/swimming because the AABB shortens.
    std::printf("torso / chest Y-anchor\n");
    {
        const float kRatio = WingsModule::kTorsoAnchorRatio;
        check(kRatio > 0.5f && kRatio < 1.0f,
              "torso anchor ratio sits between the feet and the head");
        // 1.8m standing player, AABB [0, 0, 0]..[0.6, 1.8, 0.6]
        const float height = 1.8f;
        const float feetY = 0.0f;
        const float torsoY = feetY + height * kRatio;
        // 21 px = 1.3125 blocks, so a 0.75 ratio over 1.8 blocks (1.35)
        // lands within ~0.04 blocks of the model-space chest pivot.
        const float modelChestY = 21.0f / 16.0f;
        check(std::fabs(torsoY - modelChestY) < 0.10f,
              "standing player: torso anchor lands within 0.10 blocks of model chest pivot (21 px)");
        // Sneaking shortens the AABB. The ratio of the *new* height must
        // still hit the chest, not the head or the feet.
        const float sneakHeight = 1.5f;
        const float sneakTorsoY = feetY + sneakHeight * kRatio;
        check(sneakTorsoY > 1.0f && sneakTorsoY < sneakHeight,
              "sneaking: torso anchor sits between the feet and the sneak eye");
        // The X/Z anchor is still the AABB center, which is the body
        // transform origin in the actor's local space.
        const float ax = (0.0f + 0.6f) * 0.5f;
        const float az = (0.0f + 0.6f) * 0.5f;
        check(std::fabs(ax - 0.3f) < 1e-4f && std::fabs(az - 0.3f) < 1e-4f,
              "torso anchor X/Z stay at the AABB center (body origin)");
    }
